...
const SearchFilters = ({ className = '' }) => {
      ...
      items: (variant = 'desktop') => (
        <div className={`${variant === 'mobile' ? 'w-full' : 'w-72 sm:w-80'} p-4 space-y-3`}>
          <div className="flex items-center justify-between">
            <p className="text-base font-semibold text-[var(--color-text)]">Keyword Search</p>
          ...
          <div className="relative">
            ...
            />
            {keyword && (
              <button type="button" onClick={resetKeyword} className="absolute inset-y-0 right-3 text-xs font-medium text-[var(--color-muted)] hover:text-primary">Clear</button>
            )}
          </div>
        </div>
      ),
        ...
        items: (variant = 'desktop') => (
            <div className={`${variant === 'mobile' ? 'w-full' : 'w-[min(90vw,420px)]'} p-5 space-y-6`}>
                <div className="flex items-start justify-between gap-3">
                    <div><p className="text-base font-semibold text-[var(--color-text)]">Make</p></div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    {carMakes.map(make => {
                        ...
                        return (
                            <button key={make.label} type="button" onClick={() => handleMakeToggle(make.label)} className={`flex items-center gap-2 p-2 border rounded-lg ${isActive ? 'border-primary bg-primary/5' : 'border-[var(--color-border-subtle)]'}`}>
        ...
        <div className={`${variant === 'mobile' ? 'w-full' : 'w-[min(90vw,490px)] sm:w-[min(100vw,600px)]'} p-5 space-y-5`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {Object.entries(carBodyTypes).map(([key, config]) => {
                ...
                return (
                    <button key={key} type="button" onClick={() => handleBodyTypeToggle(key)} className={`flex flex-col items-center justify-center border rounded-lg py-6 text-sm font-medium transition-all duration-200 ${isActive ? 'border-primary text-primary shadow-sm bg-primary/5' : 'border-[var(--color-border-subtle)] text-[var(--color-muted)] hover:border-primary/40'}`}>
...

    {
      ...
      items: (variant = 'desktop') => (
        <div className={`${variant === 'mobile' ? 'w-full' : 'w-[min(90vw,420px)]'} p-5 space-y-5`}>
          <div className="flex items-center justify-between">
            <div><p className="text-base font-semibold text-[var(--color-text)]">Price Range</p></div>
          ...
          <div className="mt-4">
            <div className="relative h-8" ref={priceSliderTrackRef}>
              ...
              <div className="absolute top-1/2 w-4 h-4 bg-[var(--color-surface)] border-2 border-primary rounded-full -translate-y-1/2" style={{ left: `calc(${minPercent}% - 8px)` }} onPointerDown={startPriceDrag('min')} tabIndex={0} />
              <div className="absolute top-1/2 w-4 h-4 bg-[var(--color-surface)] border-2 border-primary rounded-full -translate-y-1/2" style={{ left: `calc(${maxPercent}% - 8px)` }} onPointerDown={startPriceDrag('max')} tabIndex={0} />
            </div>
      ...
      items: (variant = 'desktop') => (
        <div className={`${variant === 'mobile' ? 'w-full' : 'w-[min(90vw,480px)]'} p-5 space-y-6`}>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-base font-semibold text-[var(--color-text)]">Year Range</p>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {fuelOptions.map(option => {
                ...
                return (
                  <button key={option.key} type="button" onClick={() => handleFuelTypeToggle(option.key)} className={`flex items-center gap-2 p-3 border rounded-lg transition-all ${isActive ? 'border-primary bg-primary/5 text-primary' : 'border-[var(--color-border-subtle)] text-[var(--color-muted)] hover:border-primary/40'}`}>
  ...
  return (
    <>
      <div ref={containerRef} className={`${className} w-full max-w-5xl bg-[var(--color-surface)] border border-[var(--color-border-subtle)] rounded-full flex items-center px-1 py-1 relative z-10`}>
          ...
          <div className="w-full self-stretch flex-1 flex items-center justify-between gap-2 relative">
              <div className="md:hidden w-full h-full px-2">
                <input
                  ...
                  onChange={(e) => setKeyword(e.target.value)}
                  placeholder="Search..."
                  className="w-full h-full bg-transparent border-none text-sm text-[var(--color-text)] placeholder:text-[var(--color-muted)] focus:ring-0 focus:outline-none p-0"
                />
              </div>    
                  ...
                  {menuItems.map(menu => (
                      <div key={menu.key} className="relative w-full justify-center items-center hidden md:flex" onMouseEnter={() => handleDesktopMouseEnter(menu.key)} onMouseLeave={handleDesktopMouseLeave}>
                          <button ref={el => buttonRefs.current[menu.key] = el} className={`flex items-center justify-center md:justify-start w-full px-2 py-3 transition-all duration-200 ${hoveredDropdown === menu.key ? 'bg-sky rounded-full' : 'text-text'}`} onClick={() => handleMenuButtonClick(menu.key)} type="button">
                              ...
                              <ChevronDown className={`${hoveredDropdown === menu.key ? 'rotate-180' : ''} size-3 text-[var(--color-muted)] ml-auto hidden md:inline-flex transition-transform`} />
                          </button>
                      </div>
                  ))}
              </nav>
      ...
      <AnimatePresence>
        {isMobileFiltersOpen && (
            <motion.div className="fixed inset-0 z-9999 bg-black/40 backdrop-blur-sm md:hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <motion.div className="absolute inset-0 bg-[var(--color-surface)] rounded-t-3xl pb-5 overflow-y-auto" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}>
                    <div className="py-6 flex items-start justify-between gap-4 px-4 sticky top-0 bg-[var(--color-surface)] pb-4 z-10 border-b border-gray-100">
                        ...
                        <button type="button" onClick={() => setIsMobileFiltersOpen(false)} className="p-2 rounded-full bg-[var(--color-elevated)] text-[var(--color-muted)]"><X className="size-4" /></button>
                    </div>
                    <div className="">
                        {menuItems.map(menu => (
                            <div key={`${menu.key}-mobile`} className=" border-b py-4 border-[var(--color-border-subtle)]">
                                {menu.items('mobile')}
                            </div>
                        ))}
                    </div>
